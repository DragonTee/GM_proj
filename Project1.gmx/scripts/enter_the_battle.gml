...
for(global.index = 1; global.index <= global.ship_count; global.index++)
{
    ...
    instance_create(room_width / 2 - cos(degtorad(global.battle_angle)) * global.battle_offset - cos(degtorad(global.battle_angle)) * global.ship_x, room_height / 2 + sin(degtorad(global.battle_angle)) * global.battle_offset + sin(degtorad(global.battle_angle)) * global.ship_y, global.ship_obj);
}
show_message(global.ship_count);
ini_close();
